...
for i in range(1,11):
    ...
    if is_boy == 1:
        ...
        if  one_or_two == 1:
            #single _names
            picked_name =random.choice(boys_given_name1)
        else:
            #double names
            picked_name =random.choice(boys_given_name2)
    else:
        #girl names
        if  one_or_two == 1:
            #single _names
            picked_name =random.choice(girls_given_name1)
        else:
            #double names
            picked_name =random.choice(girls_given_name2)

    print('\ndebug-- now given names is:%s '%(picked_names))
    full_names = one_name+picked_name
    print('\nFinal--now full name is :%s'%(full_name))
    f1.write(full_names)
    f1,write('\n')
    # --- fbe
